...
export default function AdminCategoriesPage() {
  ...
  const openNew = () => {
    ...
    resetImage(); setEditId(null); setShowForm(true);
  };
  ...
  const openEdit = (c: any) => {
    ...
    setImagePreview(c.image || ''); setImageFile(null);
    setEditId(c.id || c._id); setShowForm(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.nameAr.trim() || !form.nameEn.trim()) {
      showToast('يرجى إدخال اسم القسم بالعربية والإنجليزية', 'error'); return;
    }
    setSaving(true);
    try {
      ...
      if (imageFile) {
        ...
        try { imageUrl = await uploadToCloudinaryDirect(imageFile); }
        catch (err: unknown) { showToast((err as Error).message || 'فشل رفع الصورة', 'error'); return; }
      ...
      const payload: Record<string, unknown> = {
        ...
        slug: cleanSlug, slugAr: cleanSlug, slugEn: cleanSlug,
        status: form.status, isActive: form.status === 'active',
        image: imageUrl || null, imageAlt: form.imageAlt.trim() || null,
      };
      if (editId) {
      ...
      setShowForm(false); resetImage(); load();
    } catch (err: unknown) {
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try { await categoriesApi.delete(id); showToast('تم حذف القسم', 'success'); load(); }
    catch (e: unknown) { showToast((e as Error).message || 'فشل الحذف', 'error'); }
  };
      ...
      render: (c: any) => (
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          {c.image ? (
            ...
            <div style={{ width: 40, height: 40, borderRadius: 8, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.1rem', background: 'rgba(210,181,106,.08)', border: `1px dashed ${BORDER}` }}>🖼</div>
          )}
          <div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem', flexWrap: 'wrap' }}>
              <div onClick={() => fileInputRef.current?.click()} style={{
                ...
                border: `2px dashed ${imagePreview ? GOLD : BORDER}`, cursor: 'pointer', position: 'relative',
                background: imagePreview ? 'transparent' : 'rgba(210,181,106,.04)',
                display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'border-color 200ms',
              }}>
                {imagePreview ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2">
              <AdminInput
                ...
                value={form.imageAlt} onChange={set('imageAlt')}
                placeholder="مثال: كرسي بين باج جلد للاسترخاء — رواقة" dir="rtl"
              />
