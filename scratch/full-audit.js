...
async function runTestSuite() {
  ...
  try {
    ...
    // Verify in MongoDB
    const dbCat = await db.collection('categories').findOne({ slug: catSlug });
  ...
  // 4.5 Price Security Test: Attempt client-side price manipulation
  // Note: Checkout does NOT accept prices in payload; it calculates strictly from DB products.
  // We verify that even if a hacked body sends a fake price, backend ignores it.
  const initialAvailableStock = (await db.collection('products').findOne({ _id: new mongoose.Types.ObjectId(testProductId) })).inventory.availableQuantity;

  // 4.6 Successful COD Checkout with Coupon
  ...
  try {
    ...
    // 4.7 DB State Verification after Order Creation
    ...
    assert(dbOrder.shippingAddress.recipientName === 'أحمد محمود التست', 'Arabic customer address correctly saved');

    // 4.8 Atomic Inventory Reservation Check in DB
    ...
    // 4.9 Coupon usage counter increment check in DB
    ...
    // 4.10 Idempotency Check: Resubmitting identical request
    const retryRes = await axios.post(
      `${API}/checkout`,
      {
        cartId,
        shippingAddress: { recipientName: 'Retry Name', phone: '01098765432', streetAddress: 'Retry', city: 'Cairo', governorate: 'cairo' },
        paymentMethod: 'cash_on_delivery',
      },
      {
        headers: {
  ...
  try {
    ...
    assert(publicSettings.data.data.themeName === 'imperial_plum', 'Public Settings API immediately serves new theme colors');
  } catch (err) {
